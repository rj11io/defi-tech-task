...
const { generateToken, clearTokens, changePasswordSecret } = require('../helpers/auth');
const { langs, defaultLang } = require('../config');

const dbUser = email => User.findOne({ email }).setOptions({ internalGet: true });
const decodeChangeToken = token => jwt.verify(Buffer.from(token, 'base64').toString(), changePasswordSecret());
...
exports.changePassword = async ({ params: { email, token }, body: { password } }, res, next) => {
  try {
    ...
    if (decoded.email !== email || decoded.purpose !== 'password-reset') return next(Unauthorized());
    const user = await dbUser(email);
